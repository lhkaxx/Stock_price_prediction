"""
model_gru.py — GRU 模型定义与训练
双层 GRU + Dropout + Dense，EarlyStopping 保留最优权重
"""
import os
import numpy as np
import torch
...
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.gru1(x)                       # (batch, 30, 64)
        x, _ = self.gru2(x)                       # (batch, 30, 32)
        x = x[:, -1, :]                            # 取最后时间步
        x = self.dropout(x)
        x = self.fc(x)                             # (batch, 1)
        return x.squeeze(-1)
...
def train_model(model: nn.Module,
                ...
                lr: float = 0.001,
                patience: int = 10,
                save_path: str = "models/gru_best.pt",
    ...
    print(f"[GRU]  开始训练 | 设备:{device} | 样本:{X_train.shape[0]} | epochs:{epochs}")
    print(f"[GRU]  参数量: {sum(p.numel() for p in model.parameters()):,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            if patience_counter >= patience:
                print(f"[GRU]  Early Stopping 触发于 epoch {epoch}，最优 epoch={best_epoch} val_loss={best_val_loss:.6f}")
                break

    print(f"[GRU]  训练完成 | best_epoch={best_epoch} | best_val_loss={best_val_loss:.6f}")
    return {"train_losses": train_losses, "val_losses": val_losses, "best_epoch": best_epoch}


# ==================== 主入口 ====================

def run(data_dir: str = "data", save_path: str = "models/gru_best.pt") -> dict:
    """完整 GRU 训练流程：加载数据 → 训练 → 保存模型"""
    ...
    np.savez("models/gru_history.npz",
